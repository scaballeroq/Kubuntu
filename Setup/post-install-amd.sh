#!/bin/bash
# post-install-amd.sh - Post-instalación para Kubuntu con AMD Ryzen y AMD Graphics
# (Configurado con repositorios, microcódigo AMD, Mesa Vulkan/RADV/VA-API, ZRAM, codecs)

set -euo pipefail

echo "================================================================="
echo "INICIANDO POST-INSTALACIÓN: KUBUNTU (KDE PLASMA) - AMD RYZEN"
echo "================================================================="
...
# 5. Microcódigo y Firmware AMD
echo "ℹ️ [4/10] Instalando microcódigo AMD y firmware oficial..."
sudo apt install -y \
    amd64-microcode \
    linux-firmware

# 6. Stack Gráfico AMD (Mesa / RADV / VA-API / Vulkan)
echo "ℹ️ [5/10] Instalando controladores gráficos AMD Mesa (RADV/RadeonSI)..."
sudo apt install -y \
    mesa-va-drivers \
    mesa-vdpau-drivers \
    vulkan-tools \
...
if command -v wine &>/dev/null || command -v proton &>/dev/null; then
    ...
    sudo apt install -y lib32vulkan1 lib32mesa0 2>/dev/null || true
fi
...
echo "================================================================="
echo "✅ Kubuntu (AMD Ryzen) configurado con éxito."
