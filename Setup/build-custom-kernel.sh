#!/bin/bash
# build-custom-kernel.sh - Compilación del Kernel Linux completo optimizado para x86_64-v3 (Kubuntu / Ubuntu)
...
if [ -f "/boot/config-$(uname -r)" ]; then
    ...
    BASE_CONFIG=$(ls -1t /boot/config-*generic* /boot/config-* 2>/dev/null | grep -v "\-v3" | head -n1 || true)
...
# Desactivar firma de módulos y limpiar certificados de distribución para evitar errores con OpenSSL/sign-file
